...
export class FieldSchema<T = unknown> {
  ...
  requiredWith(field: string) {
    this.optionalField = true
    this.rules.push({ name: 'required_with', validate: (value, ctx) => !isMissing(ctx.data[field]) ? !isMissing(value) : true })
    return this
  }
  requiredWithout(field: string) {
    this.optionalField = true
    this.rules.push({ name: 'required_without', validate: (value, ctx) => isMissing(ctx.data[field]) ? !isMissing(value) : true })
    return this
  }
  ...
  size(value: number) { this.rules.push({ name: 'size', validate: actual => String(actual).length === value || Number(actual) === value }); return this }
  ...
  url() { this.rules.push({ name: 'url', validate: value => { try { new URL(String(value)); return true } catch { return false } } }); return this }
  ...
  ip() { this.rules.push({ name: 'ip', validate: value => /^[\da-f:.]+$/i.test(String(value)) }); return this }
  ...
  in(values: unknown[]) { this.rules.push({ name: 'in', validate: value => values.includes(value) }); return this }
  notIn(values: unknown[]) { this.rules.push({ name: 'not_in', validate: value => !values.includes(value) }); return this }
